    ...
    $.fn.ulynlistPagination = function (opts) {
        ...
        opts.afterPaginationRender = function (ulynlistObj,options) {
            ...
            $(document).click(function(event){
                var select_page_bar = $('.select_page_bar');
                var setGoToPage = $('.setGoToPage');
                //点击区域不属于该区域并且也不是该区域的子集 则隐藏
                if(!select_page_bar.is(event.target) && select_page_bar.has(event.target).length === 0
                    &&!setGoToPage.is(event.target) && setGoToPage.has(event.target).length === 0){
                    if(!select_page_bar.is(":hidden")){
                        select_page_bar .hide();
                    }
                }
                var page_size_bar = $('.page_size_bar');
                var tableBarPageSize = $('.js-tableBarPageSize');
                if(!page_size_bar.is(event.target) && page_size_bar.has(event.target).length === 0
                    &&!tableBarPageSize.is(event.target) && tableBarPageSize.has(event.target).length === 0){
                    if(!page_size_bar.is(":hidden")){
                        page_size_bar .hide();
                    }
                }
            });
