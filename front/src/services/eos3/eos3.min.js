...
(function (a) {
    ...
    if (!Promise) {
        ...
        var publish = function (val) {
            var promise = this, fn, st = promise._status === FULFILLED, queue = promise[st ? "_resolves" : "_rejects"];
        ...
        Promise.all = function (promises) {
            ...
            return Promise(function (resolve, reject) {
                var i = 0, result = [], len = promises.length;
    ...
    function ajax(opt, context) {
        return new Promise(function (resolve, reject) {
            var url = opt.url || "";
            var async = opt.async !== false, method = opt.type || "GET", data = opt.data || null, dataType = opt.dataType, beforeSend = opt.beforeSend || noop, complete = opt.complete || noop, success = function (data, xhr) {
                if (data.status) {
    ...
    function getContextPath(baseUrl) {
        ...
        if (scripts && scripts.length > 0) {
            for (var i = 0; i < scripts.length; i++) {
                ...
                var jsPath = nodeScript.hasAttribute ? nodeScript.src : nodeScript.getAttribute("src", 4);
    ...
    function eosPromise(opts) {
        ...
        var vars = "eos_appid=" + option.appId + "&eos_service_id=" + option.serviceId + "&eos_method_name=" + option.method + "&eos_version=" + option.version + "&eos_mock=" + (option.mock == null ? "" : option.mock);
    ...
    Service.prototype.registerMethod = function (method, v, paramNames) {
        if (isArray(paramNames)) {
            var service = this;
            // if (service[method]) {
            //     throw new Error("【eos3】服务方法已经注册，请不要重复注册：" + service.appId + "-" + service.serviceId + "-" + method)
            // }
            ...
            throw new Error("【eos3】注册方法错误，入参不正确！paramNames 必须为数组方式：" + paramNames)
        }
    };
    function Eos() {
    }
    ...
    Eos.prototype.defaultSettings = {
        ...
        error: function (msg, xhr) {
            console && console.error("调用服务异常:" + msg, xhr)
        }
    };
    ...
    Eos.prototype.registerService = function (appId, serviceId) {
        if (this[appId]) {
            if (!this.hasOwnProperty(appId)) {
                ...
                throw new Error("【eos3】不允许使用关键字[" + arr.join(",") + "]作为应用id[" + appId + "]，请更换!")
            }
        }
        var app = this[appId] = this[appId] || {};
        // if (app[serviceId]) {
        //     throw new Error("【eos3】不允许重复注册服务[" + appId + "-" + serviceId + "]，请统一一处注册")
        // }
