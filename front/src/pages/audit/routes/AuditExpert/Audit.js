...
import {findBasicFileList,submitDeclare,queryConfigTree,queryCommentClass} from '@/services/requestFunc';
...
class Audit extends BaseComponent {
    ...
    constructor(props) {
        super(props);
        let {style,businessKey,declareId,status} = props.match.params || ''; //是审批还是详情style-add/detail，审批流程对应的id--businessKey，学校id--declareId
    ...
    // 获取专家点评详情
    getConfigTree = (businessKey)=>{
        let _this = this;
        let {serviceName,methodName} = api['queryCommentClass'];
        SchemaService.callService(
            serviceName,
            methodName,
            [{"name":"jsonObject","data":{"bussinessKey" :businessKey}}]
        ).then(function (res) {
            res = res ? JSON.parse(res) : {};
            if(res && res.status && res.data){
                _this.setState({
    ...
    render() {
        let {style,businessKey,declareId,selectButtonVo,processData,scoreModal,evaluationModal,scoreData,scoreStatus,starData,treeData,commentList} = this.state;
        return (
            <div className="audit-expert">
                {/* 申报相关信息 */}
                ...
                <SelectedExpert
                        ...
                        handleDetail={
                            ()=>{
                                this.toPath(`/expertComment/${businessKey}/${declareId}/detail`)
                            }
                        }
                        title={'专家点评'}
                        expertDetail={true}></SelectedExpert>
